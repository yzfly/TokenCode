...
一个跑在终端里的编码 agent：你说一句话，它用 `read` / `write` / `edit` / `bash` 四个工具读改文件、跑命令，循环直到把活干完。核心是标准的 tool-use 循环：
...
- `protocol` 只有两种：`anthropic` 和 `openai-chat`。DeepSeek/Kimi/Qwen/OpenRouter/Ollama 都能走 `openai-chat`，换 `base_url` 即可，零代码。
