...
一个跑在终端里的**编码 agent**：单 agent tool-use 循环底座 + Bubble Tea TUI，工具有 read/write/edit/bash + ls/glob/grep（文件系统检索）+ git_status/git_diff/git_commit（git 专用）+ websearch/webfetch（联网搜索，免 key）+ cron_create/cron_list/cron_delete（会话内定时任务），可经 agent/workflow 同步委托、或 spawn_agent/wait_agent/resume_agent/list_agents 异步委托子代理与脚本化编排。**并行内核已开建**：`/race <N> <任务>` 派最多 1000 个 agent 各占一个 git worktree 独立竞赛解题，裁判流水线择优，确认后落地（ROADMAP 阶段 2 · A 的 v1）。另有**多模型协议转换层**（anthropic / openai / google）、**SSE 流式**、**会话 JSONL 持久化**、**心跳 + 自动做梦**。整张架构图见 `docs/architecture.md`（本地，未入库）。
...
- 内置工具：`read` / `write` / `edit` / `bash` + **文件系统检索**：`ls`（列目录）/ `glob`（通配找文件，`**` 递归、最近改动在前）/ `grep`（正则搜内容，跳过 .git/二进制/超大文件）+ **git 专用**：`git_status` / `git_diff`（只读）/ `git_commit`（写类，走确认）+ **联网**：`websearch`（DuckDuckGo → Mojeek 自动回退，免 key 免费）/ `webfetch`（取网页转纯文本）。只读工具集（read/ls/glob/grep/git_status/git_diff/websearch/webfetch…）由 `tools.ReadOnly` 统一判定，plan 模式、非交互拍、headless 默认白名单共用一份语义。
- **会话内定时任务（cron 三件套）**：`cron_create`（name + every ≥1m + prompt）注册周期任务，到点把 prompt 作为一拍投进主 agent 事件队列（与心跳/梦共用 Event 原语，来源 `cron`）；`cron_list` / `cron_delete` 管理。任务只活在本进程（不落盘）；定时拍按非交互处理——只读放行、写类拒绝；上限 20 个任务防失控。
- **异步子代理四件套**：`spawn_agent` 后台启动子代理立即返回句柄 id（先派活、自己接着干），`wait_agent` 阻塞收账（一条消息里多个 wait 并行收）、`resume_agent` 在保留上下文上续聊、`list_agents` 看句柄状态；与同步 `agent` 工具共用类型表、信号量（最多 8 并发）与权限通道；子代理内禁止嵌套全部委托类工具。
...
- **Go SDK（`pkg/tokencode`，嵌入式门面）**：`go get github.com/yzfly/tokencode` 后十行代码在宿主 Go 程序里起一个 agent——`New` + Option（WithModel/WithLLM/WithTools/WithAllowedTools/WithRoot/WithMaxTokens/WithSystemPrompt/WithUsageSource），`Run`（单 turn 返最终文本，连续 Run = 多轮）/ `RunStream`（headless 同构事件流）/ `AddTool` / `History` / `Reset`。核心类型（Tool/LLM/Event/Request/Response/Message…）经 type alias 原样导出，自定义工具与自定义 LLM 不需要 import internal；白名单与根隔离复用 headless 的 GateTool 与 Registry.SetRoot，缺凭据报错透传 `auth login` 指引。fake LLM 全覆盖单测 + `examples/sdk` 离线可跑示例。
- **WebUI v0（serve 自带）**：`internal/webui` 四页——`/ui` 用量大盘（本月/今天合计卡片 + 近 30 天纯 CSS 条形图 + 本月按模型/来源 Top 10，数据走 `GET /api/usage?from=&to=`，与 `/usage` 同账本）、`/ui/chat` 极简聊天（POST /v1/run SSE 流式渲染、工具调用折叠）、`/ui/team` 团队管理（绑定/pending 列表 + 页面生成配对码 `POST /api/team/pair` + 解绑 `DELETE /api/team/binding`，与 CLI 共用 team.json）、`/ui/models` 目录与凭据状态只读。纯 html/template + 原生 JS + go:embed，零框架零 CDN；serve 经 `Mount` 回调注入路由保持解耦。安全：v0 无鉴权（页面常驻警示勿绑 0.0.0.0），写 API 双闸——回环 RemoteAddr + 本机 Host（防 DNS rebinding）。
- **A2A 协议端点（被其他 agent 发现/对话/触发）**：`internal/a2a` 手写 v1 最小子集零新依赖——`GET /.well-known/agent-card.json`（name/version/supportedInterfaces url 随 Host 推导/capabilities.streaming/skills/securitySchemes 声明 Bearer 留位）+ `POST /a2a` JSON-RPC 2.0 分发器：SendMessage（阻塞跑完回 ROLE_AGENT message，失败回 Task FAILED 形态）、SendStreamingMessage（SSE：statusUpdate WORKING → assistant_delta 映射 artifactUpdate append → 终态 statusUpdate）、GetTask（-32001 TaskNotFound）、CancelTask（ctx cancel → CANCELED），0.x 别名 `message/send`/`message/stream`/`tasks/get`/`tasks/cancel` 一并接受（生态大量客户端未迁 v1），其余方法 -32004；内存 task store（终态只写一次、1000 条 FIFO 淘汰防泄漏）；自带信号量限流（同 serve 默认）；经 Mount 与 webui 并列挂载。边界同 serve：v0 无鉴权仅回环；tool_call/tool_result 不外发。
...
internal/subagent/         子代理：类型发现 + Runner（Spawn/SpawnDef 同步 + SpawnAsync/WaitJob/ResumeJob 异步句柄，工具子集 + 根隔离）
internal/cron/             会话内定时任务：Manager（AfterFunc 定时器）+ cron_create/list/delete 三工具
...
internal/tools/            Tool interface + Registry（可绑定 per-agent 根）+ read/write/edit/bash/ls/glob/grep/git_*/websearch/webfetch + ReadOnly 只读判定
pkg/tokencode/             Go SDK 门面：New + Option → Client.Run/RunStream/AddTool/History（核心类型 alias 导出）
...
- **race v1 的取舍**：racer 在自己 worktree 内**自动放行全部工具（含 bash）**——文件工具有根隔离硬约束，bash 只有 cwd 约束没有沙箱；败者提前退钱（`race.good_enough` 够好即收）默认关闭，不开则全员跑完才裁判；无预算/花费建模；裁判单评委（无投票）。
...
- **cron 只在 TUI 交互模式生效**：定时拍走主 agent 事件队列，plain（非 tty）与 headless 不消费队列，任务到点会阻塞在投递上（与心跳同一限制）；定时拍非交互、只读放行，写类工具会被拒。
- **异步子代理句柄不落盘**：spawn_agent 的句柄与上下文只活在本进程内存，退出即失；用户 Ctrl+C 打断当前拍不会杀后台子代理（这是特性），但退出进程会。
- **对标 CC 工具清单的已知缺口（后置）**：LSP 工具（要管 language server 生命周期 + JSON-RPC，工程量大）、浏览器自动化工具组（CDP 接入）、多模态 Artifacts（读图片要 llm 层支持 image content block）、apply_patch（edit 已覆盖主要场景）、PowerShell（bash 经 sh -c，Windows 后置）。
- **活体冒烟与 pty 手动冒烟**未逐项留痕；TUI 胶水层（Bubble Tea model、glamour、bridge）无自动化测试覆盖，诚实标注。

## 下一步重点

- **race 打磨**：多视角/投票裁判、预算上限、视角变体注入（variant 已留位）。败者提前退钱已落地（`race.good_enough` 够好即收）。
